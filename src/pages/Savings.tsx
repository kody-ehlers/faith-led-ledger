...
export default function Savings() {
  const { savings, addSavings, updateSavings, removeSavings } = useFinanceStore();
          ...
          {savings.length === 0 ? (
            ...
            <div key={s.id} className="flex items-center justify-between p-4 rounded-lg border border-border bg-card">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Goal: ${s.goalAmount.toFixed(2)}</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="text-primary font-bold">${s.currentAmount.toFixed(2)}</div>
                <Button variant="ghost" onClick={() => { updateSavings(s.id, { currentAmount: s.currentAmount + 100 }); toast.success('Added $100 (demo)'); }}>+ $100</Button>
                <Button variant="ghost" className="text-destructive" onClick={() => { removeSavings(s.id); toast.success('Account removed'); }}>Remove</Button>
              </div>
            </div>
          ))}
        </CardContent>
