...
function SubscriptionCard({ entry, onEdit }: { entry: SubscriptionEntry; onEdit: (entry: SubscriptionEntry) => void }) {
  ...
  const cancelSubscription = useFinanceStore((s) => s.cancelSubscription);
  const renewSubscription = useFinanceStore((s) => s.renewSubscription);
          ...
          {(() => {
            ...
            return orderedVisible.length === 0 ? (
              ...
              <SortableCardGrid
                ...
                className="space-y-3"
                layout="list"
                renderItem={(s) => <SubscriptionCard entry={s} onEdit={(e) => { setEditing(e); setIsEditOpen(true); }} />}
              />
            );
          })()}
        </CardContent>
